...
function EditGamePage() {
  ...
  useEffect(() => {
    const fetchJuego = async () => {
      try {
        ...
        // Convierte el número de año a string para el input
        setFormData({ ...data, añoLanzamiento: String(data.añoLanzamiento) });
        setLoading(false);
      } catch (err) { setError("Error al cargar los datos."); setLoading(false); }
    };
    fetchJuego();
  }, [id]);
  ...
  const handleUpdate = async (e) => {
    e.preventDefault();
    try {
      await JuegoService.updateJuego(id, { ...formData, añoLanzamiento: Number(formData.añoLanzamiento) });
    ...
    <div className="form-container">
      ...
      <form onSubmit={handleUpdate} className="game-form">
        ...
        <div className="form-actions">
            <button type="submit" className="submit-button update-button">Guardar Cambios</button>
            <button type="button" onClick={handleDelete} className="submit-button delete-button">Eliminar Juego</button>
        </div>
      </form>
    </div>
  );
